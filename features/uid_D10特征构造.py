...
# D10 = int(delta秒/3600/24) 不对 3,16 bad case
# D10 = round(delta秒/3600/24)

# 如果是D10==0,一天内只有一笔交易的话,不能用
uid_D10 = []
for DAY in tqdm_notebook(range(32, 182 + 1)):  # 2, 182+1
    print("DAY: ", DAY)
    for D10 in range(31, min(121, DAY)):  # 1, DAY
...
uid_D10.columns = ["TransactionID", "mean_", "sum_", "cnt_"]
uid_D10.to_csv("./uid_D10_train.csv",index=False)
...
for DAY in tqdm_notebook(range(213, 395 + 1)):
    print("DAY: ", DAY)
    for D10 in range(DAY - 182, min(121, DAY - 1 + 1)):  # 212
        uid_list = list(df.loc[(df["D10"] == D10) & (df["day"] == DAY), "uid"].values)
        TransactionID_list = list(df.loc[(df["D10"] == D10) & (df["day"] == DAY), "TransactionID"].values)
...
uid_D10_test.columns = ["TransactionID", "mean_", "sum_", "cnt_"]
uid_D10_test.to_csv("./uid_D10_test.csv",index=False)
