...
if size == -1:
    ...
    NROWS = size
print("NROWS: ", NROWS)
subname = '../output/ieee_lgb_submit_uid_Dx_plus_D10pred.csv'
...
# 增加uid_D15特征
uid_D15_train = pd.read_csv("../features/train_target_encoding_D15.csv")
uid_D15_test = pd.read_csv("../features/test_target_encoding_D15.csv")
train = train.merge(uid_D15_train, on="TransactionID", how="left")
test = test.merge(uid_D15_test, on="TransactionID", how="left")

# 增加uid_D10特征
uid_D10_train = pd.read_csv("../features/train_target_encoding_D10.csv")
uid_D10_test = pd.read_csv("../features/test_target_encoding_D10.csv")
train = train.merge(uid_D10_train, on="TransactionID", how="left")
test = test.merge(uid_D10_test, on="TransactionID", how="left")

# 增加uid_D1特征
uid_D1_train = pd.read_csv("../features/train_target_encoding_D1.csv")
uid_D1_test = pd.read_csv("../features/test_target_encoding_D1.csv")
train = train.merge(uid_D1_train, on="TransactionID", how="left")
test = test.merge(uid_D1_test, on="TransactionID", how="left")

# 增加uid_D2特征
uid_D2_train = pd.read_csv("../features/train_target_encoding_D2.csv")
uid_D2_test = pd.read_csv("../features/test_target_encoding_D2.csv")
train = train.merge(uid_D2_train, on="TransactionID", how="left")
test = test.merge(uid_D2_test, on="TransactionID", how="left")

# 增加uid_D3特征
uid_D3_train = pd.read_csv("../features/train_target_encoding_D3.csv")
uid_D3_test = pd.read_csv("../features/test_target_encoding_D3.csv")
train = train.merge(uid_D3_train, on="TransactionID", how="left")
test = test.merge(uid_D3_test, on="TransactionID", how="left")

# 增加uid_D4特征
uid_D4_train = pd.read_csv("../features/train_target_encoding_D4.csv")
uid_D4_test = pd.read_csv("../features/test_target_encoding_D4.csv")
train = train.merge(uid_D4_train, on="TransactionID", how="left")
test = test.merge(uid_D4_test, on="TransactionID", how="left")

# 增加uid_D10=1到30天,train用label的数据; test用预测值的数据,计算mean_,sum_,count_的特征
uid_D10_pred_train = pd.read_csv("../features/uid_D10_isFraud_train.csv")
uid_D10_pred_test = pd.read_csv("../features/uid_D10_isFraud_test.csv")
train = train.merge(uid_D10_pred_train, on="TransactionID", how="left")
test = test.merge(uid_D10_pred_test, on="TransactionID", how="left")
